...
class Hover(BaseTask):
    ...
    def __init__(self):
        ...
        self.max_error_position = 5 # distance units
    ...
    def update(self, timestamp, pose, angular_velocity, linear_acceleration):    
        ...
        reward = 0
        if error_position < 3:
            reward += 10 - abs(error_velocity) - abs(linear_acceleration.z)
        else:
            reward += -error_position - abs(error_velocity) - abs(linear_acceleration.z)

        if error_position > self.max_error_position:
            reward -= 150.0 
            done = True       
        if timestamp > self.max_duration:
            reward += 150.0  # extra reward, agent made it to the end
            done = True
